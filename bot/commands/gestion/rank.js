...
// &rank <membre> <nom_du_rank>
module.exports = {
    name: "rank",
    aliases: [],
    description: "Attribue un rank à un membre (donne les rôles associés)",
    category: "gestion",
    usage: ["rank <membre> <nom_du_rank>"],
    run: async (client, message, args, color, prefix, footer, commandName) => {
        ...
        if (!rank) return message.reply(`Rank \`${rankName}\` introuvable. Consultez \`${prefix}ranklist\` pour voir les ranks disponibles.`);
        ...
        const embed = new Discord.EmbedBuilder()
            ...
            .setDescription(`✅ ${member} a reçu le rank **${rank.name}**.`);
        ...
        if (rankLogsChan) {
            const logEmbed = new Discord.EmbedBuilder()
                .setColor(color).setTimestamp()
                .setDescription(`${message.author} a attribué le rank **${rank.name}** à ${member}`);
