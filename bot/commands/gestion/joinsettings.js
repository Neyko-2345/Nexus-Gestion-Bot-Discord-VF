...
if (pass === false) return message.channel.send(`Vous n'avez pas la permission d'utiliser cette commande.`)
    ...
    let Embed = new Discord.EmbedBuilder()
        ...
        .addFields({ name: "MP de bienvenue", value: `${!client.db.get(`joindm_${message.guild.id}`) ? "Non configuré" : `${client.db.get(`joindm_${message.guild.id}`)}`}`, inline: true })
        .addFields({ name: `Message de bienvenue`, value: `${!client.db.get(`joinmessage_${message.guild.id}`) ? "Aucun" : txt}`, inline: true })
        .addFields({ name: `Channel de bienvenue`, value: `${!message.guild.channels.cache.get(client.db.get(`joinchannel_${message.guild.id}`)) ? "Aucun" : `${message.guild.channels.cache.get(client.db.get(`joinchannel_${message.guild.id}`))}`}`, inline: true })
        .addFields({ name: "Ghost join", value: `${channels.length <= 0 ? "Aucun" : channels.map(channel => `<#${channel}>`).join("\n")}`, inline: true })
        .addFields({ name: `Rôles membre`, value: `${joinroles.length <= 0 ? "Aucun" : joinroles.map(channel => `<@&${channel}>`).join("\n")}`, inline: true })
