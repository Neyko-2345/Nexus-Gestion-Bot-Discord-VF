...
if (pass === false) return message.channel.send(`Vous n'avez pas la permission d'utiliser cette commande.`)
            ...
            if (on) {
                ...
                .setColor(color)
                .setTitle(`Snipe pdp`)
                .addFields({ name: `Statut de snipe pdp`, value: client.db.get(`show_pic_${message.guild.id}`) ? `Activé` : `Désactivé` })
                .addFields({ name: `Salon de snipe pdp`, value: `${"<#" + client.db.get(`show_pic_${message.guild.id}`) + ">" || "Aucun"}` })	
