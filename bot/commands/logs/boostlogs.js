...
module.exports = {
    name: "boostlogs",
    aliases: [],
    description: "Configure les logs des boosts du serveur",
    category: "logs",
    usage: ["boostlogs on [salon]", "boostlogs off"],
    run: async (client, message, args, color, prefix, footer, commandName) => {
        ...
        if (args[0] === "on") {
            client.db.set(`boostlogs_${message.guild.id}`, channel.id);
            return message.reply(`Les logs de boost seront envoyés dans ${channel}.`);
        }
        if (args[0] === "off") {
            client.db.delete(`boostlogs_${message.guild.id}`);
            return message.reply(`Les logs de boost sont désactivés.`);
        }
        message.reply(`Usage : \`${prefix}boostlogs on [salon]\` ou \`${prefix}boostlogs off\``);
    }
};
