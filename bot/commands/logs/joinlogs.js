...
module.exports = {
    name: "joinlogs",
    aliases: [],
    description: "Configure les logs de membres qui rejoignent",
    category: "logs",
    usage: ["joinlogs on [salon]", "joinlogs off"],
    run: async (client, message, args, color, prefix, footer, commandName) => {
        ...
        if (args[0] === "on") {
            client.db.set(`joinlogs_${message.guild.id}`, channel.id);
            return message.reply(`Les logs d'arrivées seront envoyés dans ${channel}.`);
        }
        if (args[0] === "off") {
            client.db.delete(`joinlogs_${message.guild.id}`);
            return message.reply(`Les logs d'arrivées sont désactivés.`);
        }
        message.reply(`Usage : \`${prefix}joinlogs on [salon]\` ou \`${prefix}joinlogs off\``);
    }
};
