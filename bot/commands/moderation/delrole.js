...
module.exports = {
    name: "delrole",
    aliases: [],
    description: "Permet de supprimer un rôle à l'utilisteur mentionné",
    category: "moderation",
    usage: ["delrole <utilisateur> <role>"],
...
if (pass === false) return message.channel.send(`Vous n'avez pas la permission d'utiliser cette commande.`)
        ...
        if(!user.roles.cache.has(role.id)) return message.reply(`${user.user.username} n'a pas le rôle ${role.name}`)
        await user.roles.remove(role).then(async () => {
            await message.reply(`Le rôle ${role.name} a été retiré à ${user.user.username}`)
        })

        let Embed = new Discord.EmbedBuilder()
        .setColor(color)
        .setAuthor({ name: `${message.author.tag}`, iconURL: message.author.displayAvatarURL() })
        .setDescription(`${message.author} a retiré le rôle ${role} à ${user}`)
        .setTimestamp()
