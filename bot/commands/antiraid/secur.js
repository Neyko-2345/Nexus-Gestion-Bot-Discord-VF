...
if (pass === false) return message.channel.send(`Vous n'avez pas la permission d'utiliser cette commande.`)
    ...
    if (!args[0]) {
        ...
        .addFields({ name: `__Paramètres de l'antiraid__ : ${options.length} options`, value: `\n
...
**Lockurl vanity** : \`${client.db.get(`lockurl_vanity_${message.guild.id}`) || "aucun"}\``, inline: true })
        .addFields({ name: `__Modules de l'antiraid__ : ${modules.length+1} modules`, value: `${modules.map(m => `**${m}** : \`${client.db.get(`${m}.${message.guild.id}`) || "off"} - ${client.db.get(`sanction.${m}.${message.guild.id}`) || "derank"}\``).join('\n')}
...
**Lockurl vanity** : \`${client.db.get(`lockurl_vanity_${message.guild.id}`) || "aucun"}\``, inline: true })
        .addFields({ name: `__Modules de l'antiraid__ : ${modules.length+1} modules`, value: `${modules.map(m => `**${m}** : \`${client.db.get(`${m}.${message.guild.id}`) || "off"} - ${client.db.get(`sanction.${m}.${message.guild.id}`) || "derank"}\``).join('\n')}
**blrank** : \`${client.db.get(`blrank.${message.guild.id}`) || "off"} - ${client.db.get(`sanction.blrank.${message.guild.id}`) || "derank"} - ${client.db.get(`type.blrank.${message.guild.id}`) || "danger"}\``, inline: true })
