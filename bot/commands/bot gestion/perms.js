...
module.exports = {
    ...
    run: async(client, message, args, color, prefix, footer, commandName) => {
        ...
        .addFields({ name: "Permission 1", value: perm_1.length > 0 ? perm_1.map(r => `<@&${r}>`).join("\n") : "Aucune" })
        .addFields({ name: "Permission 2", value: perm_2.length > 0 ? perm_2.map(r => `<@&${r}>`).join("\n") : "Aucune" })
        .addFields({ name: "Permission 3", value: perm_3.length > 0 ? perm_3.map(r => `<@&${r}>`).join("\n") : "Aucune" })
        .addFields({ name: "Permission 4", value: perm_4.length > 0 ? perm_4.map(r => `<@&${r}>`).join("\n") : "Aucune" })
        .addFields({ name: "Permission 5", value: perm_5.length > 0 ? perm_5.map(r => `<@&${r}>`).join("\n") : "Aucune" })
        .addFields({ name: "Permission ticket", value: perm_ticket.length > 0 ? perm_ticket.map(r => `<@&${r}>`).join("\n") : "Aucune" })
        .addFields({ name: "Permission giveaway", value: perm_giveaway.length > 0 ? perm_giveaway.map(r => `<@&${r}>`).join("\n") : "Aucune" })
